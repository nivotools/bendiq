...
export default function App() { 
  ...
  const getBendWarnings = useCallback((bendType) => {
    ...
    if (totalBendDegrees + a > 360) {
      warnings.push({ type: 'error', msg: `Cannot exceed 360° between pull points (Current: ${totalBendDegrees + a}°).`});
    }
    return warnings;
  ...
  const exportPDF = async (p) => {
    ...
    try {
      ...
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.text('Generated by BendIQ - Professional Conduit Bending Calculator', pageWidth / 2, footerY + 6, { align: 'center' });
      doc.text('bendiq.lovable.app', pageWidth / 2, footerY + 11, { align: 'center' });
      
      // Legal disclaimer
      doc.setFontSize(6);
      doc.setTextColor(120, 120, 120);
      const disclaimer = 'Legal Disclaimer: All calculations provided by this app are for informational purposes only. Use results at your own risk. The developer assumes no liability for any physical damage, personal injury, or consequential losses resulting from the use of this application.';
            ...
            <div className={`${themeConfig.card} p-4 rounded-2xl mb-4 shadow-md`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[10px] font-black uppercase tracking-widest ${themeConfig.sub}`}>Total Run Degrees</span>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={() => { vibrate(12); setTotalBendDegrees(prev => prev + a); }} className={`flex-1 py-2 px-3 text-[9px] font-black uppercase rounded-full ${themeConfig.accentBg} text-white active:scale-95 transition-transform`}>Add Bend</button>
                <button onClick={() => { vibrate(12); setTotalBendDegrees(0); }} className={`py-2 px-3 text-[9px] font-black uppercase rounded-full bg-slate-700 text-white active:scale-95 transition-transform`}>Reset</button>
              </div>
            </div>
    ...
    <div style={appStyle} className={`min-h-screen ${themeConfig.bg} ${themeConfig.text} overflow-x-hidden p-5 pb-32 flex flex-col items-center transition-colors duration-300`}> 
      ...
      <div className="max-w-md w-full relative"> 
        ...
        <div className="w-full mt-8 flex justify-center gap-6 pb-4"> 
          ...
          <Link to="/privacy-policy" className={`text-[10px] font-bold ${themeConfig.sub} hover:${themeConfig.text} transition-colors`}>Privacy Policy</Link>
